...
from motorsports_data_notebook.suspension import (
    ...
    analyze_suspension_velocity_multi_lap,
...
class TestAnalyzeSuspensionVelocityMultiLap:
    """Tests for analyze_suspension_velocity_multi_lap function."""
    ...
    def test_empty_lap_numbers_raises(self):
        """Should raise ValueError when lap_numbers is empty."""
        timecodes = np.arange(0, 100, 10, dtype=np.int64)
        displacement = np.ones(len(timecodes))
        log = self._create_mock_log(timecodes, displacement)

        with pytest.raises(ValueError, match="lap_numbers cannot be empty"):
            analyze_suspension_velocity_multi_lap(log, [])

    def test_returns_velocity_histogram_result(self):
        """Should return VelocityHistogramResult with all four corners."""
        timecodes = np.arange(0, 1000, 10, dtype=np.int64)
        displacement = np.cumsum(np.random.randn(len(timecodes))) + 50
        log = self._create_mock_log(timecodes, displacement)

        result = analyze_suspension_velocity_multi_lap(log, [1, 2])

        assert isinstance(result, VelocityHistogramResult)
        assert result.front_left.corner_name == "FL"
        assert result.front_right.corner_name == "FR"
        assert result.rear_left.corner_name == "RL"
        assert result.rear_right.corner_name == "RR"

    def test_uses_default_channel_names(self):
        """Should use SUSPENSION_CHANNEL_NAMES when channel_names is None."""
        timecodes = np.arange(0, 500, 10, dtype=np.int64)
        displacement = np.cumsum(np.random.randn(len(timecodes)))
        log = self._create_mock_log(timecodes, displacement)

        result = analyze_suspension_velocity_multi_lap(log, [1])
        assert result is not None

    def test_missing_channel_key_raises_key_error(self):
        """Should raise KeyError if required channel key is missing from channel_names."""
        timecodes = np.arange(0, 100, 10, dtype=np.int64)
        displacement = np.ones(len(timecodes))
        log = self._create_mock_log(timecodes, displacement)

        with pytest.raises(KeyError):
            analyze_suspension_velocity_multi_lap(log, [1], channel_names={"shock_fl": "X"})

    def test_velocity_path_with_iracing_channels(self):
        """Should handle iRacing velocity channels (m/s -> mm/s conversion)."""
        velocity_data = np.array([0.05] * 50)  # 0.05 m/s = 50 mm/s
        log, channel_names = self._create_velocity_mock_log(velocity_data)

        result = analyze_suspension_velocity_multi_lap(log, [1], channel_names=channel_names)

        assert result.front_left.mean == pytest.approx(50.0)

    def test_histogram_sums_to_100(self):
        """Histogram percentages should sum to approximately 100%."""
        np.random.seed(42)
        timecodes = np.arange(0, 10000, 10, dtype=np.int64)
        displacement = np.cumsum(np.random.randn(len(timecodes))) + 50
        log = self._create_mock_log(timecodes, displacement)

        result = analyze_suspension_velocity_multi_lap(log, [1, 2, 3])

        assert result.front_left.histogram.sum() == pytest.approx(100.0, rel=0.01)

    def test_custom_velocity_ranges(self):
        """Should use custom velocity ranges when provided."""
        timecodes = np.arange(0, 1000, 10, dtype=np.int64)
        displacement = np.cumsum(np.random.randn(len(timecodes))) + 50
        log = self._create_mock_log(timecodes, displacement)

        custom_ranges = VelocityRanges(
            friction=5.0,
            slow=50.0,
            fast=100.0,
        )

        result = analyze_suspension_velocity_multi_lap(log, [1], velocity_ranges=custom_ranges)

        assert result.velocity_ranges == custom_ranges
