...
from motorsports_data_notebook._util import clean_laps_table, detect_file_type, infer_channel_scale
...
class TestCleanLapsTable:
    """Tests for clean_laps_table function."""
    ...
    def test_removes_lap_zero(self):
        ...
        result = clean_laps_table(laps, {})
        assert result.column("num").to_pylist() == [1, 2, 3]

    def test_removes_pit_markers_iracing(self):
        ...
        # LapDistPct channel that spans the full session
        ldp_table = pa.table(
            {
        ...
        result = clean_laps_table(laps, {"LapDistPct": ldp_table})
        # Pit marker (16ms) should be removed; duplicates resolved by longest
        nums = result.column("num").to_pylist()
        assert 0 not in nums
        # Each lap number should appear at most once
        assert len(nums) == len(set(nums))

    def test_removes_partial_laps_iracing(self):
        """Partial laps (didn't start/end near S/F) should be removed."""
        # 3 laps: full lap, out-lap from pit (starts at 50%), full lap
        laps = self._make_laps(
            nums=[1, 2, 3],
            start_times=[0, 60000, 120000],
            end_times=[60000, 120000, 180000],
        )
        # LapDistPct: lap 1 full (0→~1), lap 2 starts mid-track (0.5→1), lap 3 full
        tc = list(range(0, 180000, 100))
        ldp = []
        for t in tc:
            if t < 60000:
                ldp.append((t % 60000) / 60000.0)  # 0 → ~1
            elif t < 120000:
                ldp.append(0.5 + ((t - 60000) / 60000.0) * 0.5)  # 0.5 → ~1
            else:
                ldp.append(((t - 120000) % 60000) / 60000.0)  # 0 → ~1
        ldp_table = pa.table(
            {
        ...
        result = clean_laps_table(laps, {"LapDistPct": ldp_table})
        nums = result.column("num").to_pylist()
        # Lap 2 should be removed (started at 50% track position)
    ...
    def test_aim_data_no_lapdistpct(self):
        """AIM data (no LapDistPct) only removes lap 0 and deduplicates."""
        laps = self._make_laps(
            nums=[0, 1, 2, 3],
            start_times=[0, 60000, 120000, 180000],
            end_times=[60000, 120000, 180000, 240000],
