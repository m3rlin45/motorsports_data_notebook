...
def compute_curvature(x, y, pos_smooth_window=15, curv_smooth_window=30):
    ...
    if curv_smooth_window > 1:
        signed_curvature = pd.Series(signed_curvature).rolling(curv_smooth_window, center=True, min_periods=1).mean().values
...
def identify_corners_from_curvature(distance, curvature, signed_curvature, threshold=0.006, 
            ...
            if corner_start is None:
                ...
                if corner_length >= min_corner_length:
                    ...
                    corners.append(Corner(
                        ...
                        direction='L' if corner_direction > 0 else 'R',
    ...
    for corner in corners:
        if (merged_corners and 
            (corner.start_dist - merged_corners[-1].end_dist) < min_gap and
            corner.direction == merged_corners[-1].direction):
...
def merge_accel_zones_by_time(
    ...
    return [tuple(z) for z in merged]
