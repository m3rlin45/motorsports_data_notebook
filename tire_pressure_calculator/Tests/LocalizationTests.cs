...
public class LocalizationTests
{
    ...
    [Fact]
    public void Localizer_LanguageChangeFiresItemIndexerNotification()
    {
        Localizer.Instance.SetPreference("en");
        bool itemBracketsFired = false;
        Localizer.Instance.PropertyChanged += handler;
        try
        {
            Localizer.Instance.SetPreference("ja");
            Assert.True(itemBracketsFired,
                "Expected PropertyChanged(\"Item[]\") so XAML bindings refresh on language change.");
        }
        finally
        {
            Localizer.Instance.PropertyChanged -= handler;
        ...
        {
            if (e.PropertyName == "Item[]") itemBracketsFired = true;
        }
    }
}
